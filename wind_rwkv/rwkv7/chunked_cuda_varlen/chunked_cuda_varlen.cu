...
__global__ void forward_kernel(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_, F_ s0_, bf* y_, bf* s_, bf* sT_, long long*cu_seqlens) {
    constexpr int C = _C_, K = 16;
    int bi = blockIdx.y, hi = blockIdx.x;
    int t_off = cu_seqlens[bi]/K, nT = cu_seqlens[bi+1]/K - t_off;
    ...
    auto push = [&](int t) {
        int off = (t_off + t)*K*H*C + hi*C + warpi*16;
        int si = t%fw_stages;
    ...
    for (int t = 0; t < fw_stages-1 && t < nT; t++) push(t), __commit_group();
    ...
    for (int t = 0; t < nT; t++) {
        ...
        for (int i = 0; i < WARPS; i++) {
            if (s_ != NULL) {
                int off = hi*(T/K)*C*C + (t_off+t)*C*C + warpi*16*C + i*16;
...
void cuda_forward(int B, int T, int H, bf*w, bf*q, bf*k, bf*v, bf*a, bf*b, bf*s0, bf*y, bf*s, bf*sT, long long*cu_seqlens) {
    ...
    forward_kernel<<<dim3(H,B), dim3(threads), shared_mem>>>(T,H,w,q,k,v,a,b,s0,y,s,sT,cu_seqlens);
}


__global__ void backward_kernel(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_, F_ dy_, F_ s_, F_ dsT_, bf* dw_, bf* dq_, bf* dk_, bf* dv_, bf* da_, bf* db_, bf* ds0_, long long*cu_seqlens) {
    constexpr int C = _C_, K = 16;
    int bi = blockIdx.y, hi = blockIdx.x;
    int t_off = cu_seqlens[bi]/K, nT = cu_seqlens[bi+1]/K - t_off;
    extern __shared__ char smem_[];
    ...
    auto push = [&](int t) {
        ...
        for (int i = 0; i < WARPS; i++) {
            int off2 = hi*(T/K)*C*C + (t_off+t)*C*C + warpi*16*C + i*16;
    ...
    for (int t = 0; t < bw_stages-1 && t < nT; t++) push(nT-1-t), __commit_group();
...
void cuda_backward(int B, int T, int H, bf*w, bf*q, bf*k, bf*v, bf*a, bf*b, bf*dy, bf*s, bf*dsT, bf*dw, bf*dq, bf*dk, bf*dv, bf*da, bf*db, bf*ds0, long long*cu_seqlens) {
    ...
    backward_kernel<<<dim3(H,B), dim3(threads), shared_mem>>>(T,H,w,q,k,v,a,b,dy,s,dsT,dw,dq,dk,dv,da,db,ds0,cu_seqlens);
}
